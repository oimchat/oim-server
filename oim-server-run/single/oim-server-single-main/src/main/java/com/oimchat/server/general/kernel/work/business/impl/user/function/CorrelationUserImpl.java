...
public class CorrelationUserImpl implements CorrelationUser {
	...
	public void loadCorrelationUserIds(String userId, CorrelationUserIdStream stream) {
		...
		if (null != userId) {
			List<ContactRelation> contactList = contactRelationDAO.getListByContactUserId(userId);
			...
			for (GroupMember gm : inMemberList) {
				List<GroupMember> groupMemberList = groupMemberDAO.getListByGroupId(gm.getGroupId());
