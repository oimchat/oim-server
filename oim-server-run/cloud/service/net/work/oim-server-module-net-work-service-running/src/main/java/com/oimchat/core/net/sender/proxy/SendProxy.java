...
package com.oimchat.core.net.sender.proxy;
...
import com.oimchat.core.net.sender.annotation.DataKey;
import com.oimchat.core.net.sender.handler.DataSendHandler;
...
/**
 * Description <br>
 * Date 2020-04-03 14:12:19<br>
 * 
 * @author XiaHui [onlovexiahui@qq.com]<br>
 * @since 1.0.0
 */

public class SendProxy implements InvocationHandler, Serializable {
	...
	public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
		...
		String dataKey = "";
		DataBackAction dataBackAction = null;
		...
		if (null != args) {
			...
			for (int i = 0; i < length; i++) {
				...
				if (null != p) {
					Define d = p.getAnnotation(Define.class);
					if (null != d) {
						String key = d.value();
						if (null != key && !key.isEmpty()) {
							if (key.startsWith(bodyPut)) {
								key = key.substring(bodyPut.length());
					...
					DataKey dk = p.getAnnotation(DataKey.class);
					if (null != dk) {
						dataKey = (null == data) ? "" : data.toString();
					}

					if (data instanceof DataBackAction) {
						dataBackAction = (DataBackAction) data;
					}
				}
			}
		}

		ActionMapping cam = clazz.getAnnotation(ActionMapping.class);
		ActionMapping mam = method.getAnnotation(ActionMapping.class);
		if (null != cam) {
			String[] vs = cam.value();
			actionCode = (null == vs || vs.length == 0) ? "" : vs[0];
		}
		if (null != mam) {
			String[] vs = mam.value();
			methodCode = (null == vs || vs.length == 0) ? "" : vs[0];
		}
		AbstractMessage<?> message = null;
		if (null != bodyData) {
			message = PushMessageBuilder.buildBody(dataKey, actionCode, methodCode, bodyData);
		} else {
			message = PushMessageBuilder.buildBody(dataKey, actionCode, methodCode, dataMap);
		}

		if (null != dataSendHandler && null != message) {
			dataSendHandler.send(dataKey, message, dataBackAction);
		}
		...
		String body = null != message ? JSONObject.toJSONString(message) : null;
		...
		if (MessageValue.class.equals(rawType)) {
			...
			returnData = JSONObject.parseObject(body, returnType);
			MessageValue<?> md = (MessageValue<?>) returnData;
			md.setJsonData(body);
			return returnData;
		}
	...
	public DataSendHandler getDataSendHandler() {
		return dataSendHandler;
	}

	public void setDataSendHandler(DataSendHandler dataSendHandler) {
		this.dataSendHandler = dataSendHandler;
	}
}
