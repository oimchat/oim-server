...
package com.oimchat.server.general.kernel.net.module.oc.impl;
...
import com.oimchat.core.net.back.AbstractParallelDataBackAction;
import com.oimchat.core.net.back.ParallelDataBackAction;
...
public class ReceiveHandlerImpl implements ReceiveHandler {
	...
	public void receive(Object data, Map<String, HandleData> handleDataMap) {
		...
		if (JsonUtil.maybeJson(text)) {
			Object value = JSONPath.read(text, "head.key");
			String key = (null == value) ? "" : value.toString();
			HandleData hd = handleDataMap.remove(key);
			boolean parallel = true;

			if (null != hd && null != hd.getDataBackAction()) {
				DataBackAction dataBackAction = hd.getDataBackAction();
				if ((dataBackAction instanceof AbstractParallelDataBackAction) || (dataBackAction instanceof ParallelDataBackAction)) {
					parallel = true;
				} else {
					parallel = false;
				}
					...
					for (Method method : methods) {
						...
						for (Annotation a : as) {
							if (a instanceof Back) {
								...
								ActionRequest actionRequest = generalMessageHandler.getActionDispatcher().createActionRequest("", dataBackAction, method, data);
								ArgumentBox beanBox = new DefaultArgumentBox();
								generalMessageHandler.getActionDispatcher().action(actionRequest, (d) -> {
								}, beanBox);
								break;
							}
						}
...
//				generalMessageHandler.doMessage("", data, actionResponse, argumentBox);
			}
		}
	}
}
