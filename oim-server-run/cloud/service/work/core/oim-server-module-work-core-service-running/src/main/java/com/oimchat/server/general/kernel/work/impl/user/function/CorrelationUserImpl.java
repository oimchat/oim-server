...
import com.oimchat.server.general.common.api.inside.work.contact.ContactRelationInterface;
...
/**
 * Description <br>
 * Date 2020-04-09 19:07:44<br>
 * 
 * @author XiaHui [onlovexiahui@qq.com]<br>
 * @since 1.0.0
 */
@Component
public class CorrelationUserImpl implements CorrelationUser {

	@Resource
	private ContactRelationInterface contactRelationInterface;
	@Resource
	private GroupMemberClient groupMemberClient;

	@Override
	public void loadCorrelationUserIds(String userId, CorrelationUserIdStream stream) {

		if (null != userId) {
			List<ContactRelation> contactList = contactRelationInterface.getListByUserId(userId);
			Set<String> set = new HashSet<>();
			for (ContactRelation cr : contactList) {
				set.add(cr.getOwnerUserId());
			}
			stream.list(new ArrayList<>(set));

			MessageValue<List<GroupMember>> mv = groupMemberClient.getListByUserId(userId);

			List<GroupMember> inMemberList = mv.getBody();
			if (null != inMemberList) {
				for (GroupMember gm : inMemberList) {

					MessageValue<List<GroupMember>> mvl = groupMemberClient.getListByGroupId(gm.getGroupId());
					List<GroupMember> groupMemberList = mvl.getBody();
					if (null != groupMemberList) {
						Set<String> idSet = new HashSet<>();
						for (GroupMember m : groupMemberList) {
							idSet.add(m.getUserId());
						}
						stream.list(new ArrayList<>(idSet));
					}
				}
			}
		}
